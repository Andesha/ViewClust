...
def job_use(jobs, d_from, target, d_to='', use_unit='cpu', job_state='all',
    ...
    queued_submit = jobs_submit.groupby( pd.Grouper(freq=grouper_interval))['use_unit'].sum().fillna(0)
    queued_start  =  jobs_start.groupby( pd.Grouper(freq=grouper_interval))['use_unit'].sum().fillna(0)
    queued  = queued_submit.subtract( queued_start, fill_value=0).cumsum()
